...
HTTPS는 텍스트를 암호화한다. (TLS 핸드셰이크 시 공개키 방식으로 대칭키를 안전하게 교환한 뒤, 실제 데이터 전송은 대칭키 방식으로 암호화한다.) : [공개키 설명](https://github.com/kim6394/tech-interview-for-developer/blob/master/Computer%20Science/Network/%EB%8C%80%EC%B9%AD%ED%82%A4%20%26%20%EA%B3%B5%EA%B0%9C%ED%82%A4.md)
...
**_CA란?_** : Certificate Authority로, 디지털 인증서를 발급하고 서명하는 신뢰성이 검증된 공인 기관

3. 계약 완료된 CA 기업은 해당 기업의 이름, A서버 공개키, 공개키 암호화 방법을 담은 인증서를 만들고, 해당 인증서를 CA 기업의 개인키로 암호화해서 A서버에게 제공한다.

4. A서버는 암호화된 인증서를 갖게 되었다. 이제 클라이언트가 A서버에 HTTPS 연결을 시도하면, TLS 핸드셰이크 과정에서 A서버는 이 암호화된 인증서를 클라이언트에게 건내준다.

5. 클라이언트가 `main.html` 파일을 달라고 A서버에 요청했다고 가정하자. 클라이언트는 CA 기업의 개인키로 서명된 인증서를 전달받게 된다.
