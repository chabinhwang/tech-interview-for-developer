...
참고: 위 Java 예시에서 identityHashCode 결과가 같은 것은 primitive type이 주소값을 저장하기 때문이 아니라, autoboxing으로 생성된 Integer 객체가 -128~127 범위에서 캐시를 공유하기 때문이다. Java의 primitive type은 스택에 값 자체를 직접 저장한다.
