...
> CPU가 현재 실행 중인 태스크의 문맥을 저장하고, 다음 태스크의 문맥을 복원하는 과정

문맥에는 레지스터 값, PC, 스택 포인터, 스케줄링 정보 등이 포함된다.

오버헤드가 생기는 이유는 다음과 같다.

- 레지스터 저장/복원
- 스케줄러 실행
- 캐시/TLB/분기 예측기 지역성 손실
- 프로세스 전환 시 주소 공간 전환 비용

즉, "캐시를 초기화해서" 느린 것이 아니라 **기존에 쌓아둔 실행 문맥과 캐시 지역성이 깨지기 때문**에 느려진다.
