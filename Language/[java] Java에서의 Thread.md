...
- WAITING : 다른 스레드의 특정 작업을 기다리는 일시정지 상태
- TIMED_WAITING : 일정 시간동안 대기하는 일시정지 상태
...
start() 이후에 t.join()을 호출하면, 호출한 스레드(예: main)가 대상 스레드(t)의 종료를 기다린다.
...
- `synchronized` 키워드 : 메서드 또는 블록 단위로 임계 영역을 설정하여 한 번에 하나의 스레드만 접근하도록 함
- `volatile` 키워드 : 변수의 값을 메인 메모리에서 직접 읽고 쓰도록 하여 가시성(visibility) 보장
- `java.util.concurrent.locks.Lock` (ReentrantLock 등) : synchronized보다 유연한 락 메커니즘 제공 (tryLock, 타임아웃 등)
- `java.util.concurrent.Semaphore` : 한정된 개수의 자원을 여러 스레드가 사용하려고 할 때 접근 제한
- `wait() / notify() / notifyAll()` : Object 클래스의 메서드로, 스레드 간 협력(대기/통지) 메커니즘
- `java.util.concurrent` 패키지 : CountDownLatch, CyclicBarrier, Atomic 클래스 등 다양한 동시성 유틸리티 제공
