...
외부 인터럽트는 `CPU 외부의 하드웨어 신호에 의해 발생`

내부 인터럽트는 `CPU 내부에서 명령어 실행 중 잘못된 명령이나 데이터에 의해 발생`

소프트웨어 인터럽트는 `프로그램의 명령어 수행(시스템 콜 등)에 의해 발생`
...
- ##### 내부 인터럽트
  ...
  > **참고**: 엄밀히 말하면, Trap은 의도적인 소프트웨어 인터럽트(예: 시스템 콜)를 가리키고, 0으로 나누기 등의 오류는 Exception(예외)으로 분류하는 것이 더 정확하다. 다만 교재에 따라 내부 인터럽트 전체를 Trap으로 통칭하는 경우도 있으므로 분류 기준이 다를 수 있다.
