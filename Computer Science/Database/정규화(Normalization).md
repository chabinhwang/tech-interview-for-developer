...
가장 큰 목표는 **불필요한 데이터 중복을 줄이고 이상 현상(Anomaly)을 막는 것**이다.

중복이 줄어들면 무결성을 유지하기 쉬워지고, 저장 공간과 변경 비용도 더 효율적으로 관리할 수 있다.
...
- 릴레이션이 2NF에 만족한다.
- 기본키가 아닌 속성은 기본키에만 의존하고, 다른 일반 속성을 통해 이행적으로 종속되지 않는다.

즉, "기본키가 아닌 속성이 또 다른 기본키가 아닌 속성에 의존하는 상황"을 제거하는 단계라고 보면 된다.
