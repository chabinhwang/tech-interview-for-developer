...
1. 클라이언트는 서버에게 `client hello` 메시지를 담아 서버로 보낸다.
   이때 암호화되지 않은 정보를 함께 담는데, `TLS 버전`, `지원하는 암호 알고리즘(Cipher Suites)`, `압축 방식`, `클라이언트 난수(client random)` 등을 담는다.

   <br>

2. 서버는 클라이언트가 보낸 암호 알고리즘과 압축 방식을 받고, `세션 ID`와 `서버의 SSL/TLS 인증서`를 `server hello` 메시지와 함께 담아 응답한다. 이 인증서에는 CA가 서명한 서버의 공개키가 포함되어 있으며, 클라이언트는 이 공개키를 handshake 과정에서 암호화에 사용한다.
...
4. CA 인증서에 대한 신뢰성이 확보되었다면, 클라이언트는 pre-master secret(난수 값)을 생성하여 서버의 공개키로 암호화한 뒤 서버에 전송한다. 이 pre-master secret과 앞서 교환한 난수(client random, server random)를 조합하여 양측이 동일한 대칭 세션키를 생성한다.
