...
이런 상황에서는 퀵소트 장점이 사라지므로, 피벗을 선택할 때 `중간 요소`로 선택하면 이미 정렬된 배열에 대한 최악의 경우를 피할 수 있음. 단, 중간 요소 선택만으로 O(n^2)이 완전히 방지되지는 않으며, `랜덤 피벗 선택`이나 `중위값 3(median-of-three)` 방식이 더 안정적인 해결책임
