...
만약 L1 캐시가 128kb면, 64/64로 나누어 64kb에는 명령어를 저장하고(I-Cache, Instruction Cache), 나머지 64kb에는 데이터를 저장한다(D-Cache, Data Cache). 이처럼 명령어와 데이터를 분리하여 캐싱하는 방식을 Harvard Architecture라 한다.

- L1 : CPU 내부에 존재
- L2 : 현대 프로세서에서는 CPU 내부에 코어별 또는 코어 공유로 존재
- L3 : 현대 프로세서에서는 CPU 다이 내부에 모든 코어가 공유하는 형태로 존재
